...
public class DABListCtgAdapter extends RecyclerView.Adapter<DABListCtgAdapter.DABListAllHolder>{
    ...
    public void onBindViewHolder(@NonNull DABListAllHolder holder, int position) {
        ...
        if (type == Constant.LIST_TYPE_ENSEMBLE){
            ...
            //如果是正在播放，则显示播放状态
            //这里应该判断集合
            if (CompareUtils.isSameDABEnsemble(radioMessage,DABTrigger.getInstance().mRadioStatusTool.getCurrentRadioMessage()) && selectedPos == -1){
               holder.rlDABSecondListItem.setSelected(true);
            }else {
                if (selectedPos == position){
                    holder.rlDABSecondListItem.setSelected(true);
                }else {
                    holder.rlDABSecondListItem.setSelected(false);
                }
            }
        }else {
            ...
            //如果是正在播放，则显示播放状态
            //这里应该判断类型
            if (CompareUtils.isSameDABType(radioMessage,DABTrigger.getInstance().mRadioStatusTool.getCurrentRadioMessage()) && selectedPos == -1){
                holder.rlDABSecondListItem.setSelected(true);
            }else {
                if (selectedPos == position){
                    holder.rlDABSecondListItem.setSelected(true);
