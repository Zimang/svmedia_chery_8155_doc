...
public class DABPlayFragment extends BaseFragment implements View.OnClickListener, IDABOperationListener, CompoundButton.OnCheckedChangeListener {
    ...
    private ImageView ivDABMore;//显示更多信息
    ...
    public int getLayoutResID() {
        return isRightRudder ? R.layout.fragment_dabplay_layout_right : R.layout.fragment_dabplay_layout;
    }
    ...
    public void initView(View view) {
        ...
        ivDABMore = view.findViewById(R.id.ivDABMore);
    ...
    public void initViewListener() {
        ...
        ivDABMore.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (R.id.ivDABPre == id){
            //pre
            DABTrigger.getInstance().mRadioControl.processCommand(isRightRudder ?RadioAction.STEP_FORWARD : RadioAction.STEP_BACKWARD, ChangeReasonData.CLICK);
            //埋点：上一曲
            PointTrigger.getInstance().trackEvent(Point.KeyName.DABOperate,Point.Field.PlayOperType,Point.FieldValue.PRE
                    ...
                    ,Point.Field.RadioName,dabMessage == null ? "" : dabMessage.getProgramStationName()
                    ,Point.Field.Mhz,String.valueOf(dabMessage == null ? "-1" : dabMessage.getFrequency()));
        }else if (R.id.ivDABNext == id){
            //next
            DABTrigger.getInstance().mRadioControl.processCommand(isRightRudder ?RadioAction.STEP_BACKWARD : RadioAction.STEP_FORWARD,ChangeReasonData.CLICK);
            //埋点：下一曲
    ...
    private void updateCurrentRadio(){
        ...
        if (radioMessage.getDabMessage() == null){
            ...
            if ((radioMessage.getDabMessage().getProgramStationName() == null && radioMessage.getDabMessage().getEnsembleLabel() == null)
                ...
                mHandler.sendEmptyMessageDelayed(DABMsg.MSG_NO_SIGNAL_TIMEOUT,DABMsg.NO_SIGNAL_TIMEOUT);
            }else {
                rlDABNoSignal.setVisibility(View.GONE);
                //优先使用Sls
...
//                                .error(R.mipmap.img_play_dab);
                        Glide.with(this).load(logoList).listener(new RequestListener<Drawable>() {
                                    @Override
    ...
    private void updatePlayStatues(){
        //当前是DAB时才更新，如果不是DAB，应该处于暂停，用于Mode后台切换的场景
        boolean isPlaying = DABTrigger.getInstance().mRadioStatusTool.isPlaying() && DABTrigger.getInstance().mRadioStatusTool.getCurrentRadioMessage().getDabMessage() != null;
...
//                        ivLogo.setBackgroundResource(0);
        ...
        if (currentLogo != null && Arrays.equals(currentLogo, logoByte)){
            ...
            RequestOptions option = RequestOptions
                    .bitmapTransform(new RoundedCorners(8));
//                                .error(R.mipmap.img_play_dab);
            Glide.with(this).load(logoByte).listener(new RequestListener<Drawable>() {
                        @Override
                        public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                            im_logo.setVisibility(View.GONE);
                            ivLogo.setImageResource(R.mipmap.img_play_dab);
    ...
    public void updateViewWithSignal(boolean hasSignal){
        ...
        ivDABList.setEnabled(hasSignal);
...
//            mHandler.sendMessage(msg);
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what){
                ...
                case DABMsg.MSG_NO_SIGNAL_TIMEOUT:
                    dabPlayFragment.hideNoSignalWithTimeout();
                    break;
                default:
                    break;
            }
    ...
    private void showMoreIcon(){
        if (windowManager == null){
            ...
            viewMore = inflater.inflate(R.layout.dab_play_more,null);
            ivDABSettings = viewMore.findViewById(R.id.ivDABSettings);
            ivDABSearch = viewMore.findViewById(R.id.ivDABSearch);
            ivDABRT = viewMore.findViewById(R.id.ivDABRT);
