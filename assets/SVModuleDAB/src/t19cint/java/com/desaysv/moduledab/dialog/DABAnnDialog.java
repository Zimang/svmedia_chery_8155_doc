...
public class DABAnnDialog extends Dialog implements View.OnClickListener{
    ...
    private void init(){
        ...
        setContentView(ProductUtils.isRightRudder() ? R.layout.dab_ann_dialog_right : R.layout.dab_ann_dialog);
    ...
    public void updateAnnTitle(DABAnnNotify annNotify){
        currentAnnNotify = annNotify;
        if (tvDABAnnTitle != null && annNotify != null){
            tvDABAnnTitle.setText(changeTypeToString(annNotify.getAnnounceType()));
        }
    }

    private DABAnnNotify currentAnnNotify;
    public void reShowDialog(){
        init();
        updateAnnTitle(currentAnnNotify);
        if (isShowing()) {
            show();
        }
    }
    ...
    public void onClick(View v) {
        int id  = v.getId();
        Log.i(TAG, "onClick: dialogDismiss");
        dismiss();
    }
}
