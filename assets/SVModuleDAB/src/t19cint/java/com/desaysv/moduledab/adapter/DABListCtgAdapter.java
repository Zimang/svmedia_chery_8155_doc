...
public class DABListCtgAdapter extends RecyclerView.Adapter<DABListCtgAdapter.DABListAllHolder>{
    ...
    public DABListCtgAdapter.DABListAllHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (type == Constant.LIST_TYPE_ENSEMBLE){
            view = LayoutInflater.from(mContext).inflate(isRightRudder ? R.layout.dab_listesem_item_right : R.layout.dab_listesem_item, parent, false);
        }else {
            view = LayoutInflater.from(mContext).inflate(isRightRudder ? R.layout.dab_listctg_item_right : R.layout.dab_listctg_item, parent, false);
        }
