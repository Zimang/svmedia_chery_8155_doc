...
public abstract class BaseRemountFunction implements IMediaFunction {
    ...
    public void changeLoopType(String source) {
        Log.d(TAG, "collect: source = " + source);
        if (getRemoteManager().getInitSuccess()) {
            getRemoteManager().sendCommand(source, LocalMediaConstants.ControlAction.CHANGE_LOOP_TYPE, "");
        }
    }
    ...
    public void openSourceWithFlag(String source, boolean isForeground, String openReason, int flag) {
        if (getRemoteManager().getInitSuccess()) {
            Gson gson = new Gson();
            MediaInfoBean.Builder builder = new MediaInfoBean.Builder().setSource(source).setForeground(isForeground).setFlag(flag);
            getRemoteManager().sendCommand(source, LocalMediaConstants.ControlAction.OPEN_SOURCE, gson.toJson(builder));
        }
    }
    ...
    public void setMedia(String source, MediaInfoBean mediaInfoBean) {
        if (getRemoteManager().getInitSuccess()) {
            Gson gson = new Gson();
            getRemoteManager().sendCommand(source, LocalMediaConstants.ControlAction.SET_MEDIA, gson.toJson(mediaInfoBean));
        }
    }
