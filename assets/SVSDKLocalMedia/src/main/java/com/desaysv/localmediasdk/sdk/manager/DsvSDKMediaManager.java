...
import com.desaysv.localmediasdk.sdk.remote.MediaRemoteControl;
 ...
 */
public class DsvSDKMediaManager implements ISDKMediaManager {

    private static final String TAG = "DsvSDKMediaManager";

    private static ISDKMediaManager instance;

    public static ISDKMediaManager getInstance() {
        if (instance == null) {
            synchronized (DsvSDKMediaManager.class) {
                if (instance == null) {
                    instance = new DsvSDKMediaManager();
                }
            }
        }
        return instance;
    ...
    public void seekTo(String source, int time) {
        ...
        MediaRemoteControl.getInstance().seekTo(source, time);
    }
    ...
    public void changeLoopType(String source, String typeInfo) {
        Log.d(TAG, "changeLoopType: source = " + source + " typeInfo = " + typeInfo);
        MediaRemoteControl.getInstance().changeLoopType(source, typeInfo);
    }
    ...
    public String getDeviceConnectState(String source) {
        String connectState = MediaRemoteControl.getInstance().getDeviceConnectState(source);
        Log.d(TAG, "getDeviceConnectState: source = " + source + " connectState = " + connectState);
        return connectState;
    }
