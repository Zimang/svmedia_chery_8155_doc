...
public class RadioControlUtils {
    ...
    public void changeCollect(String source) {
        ...
        switch (source) {
            ...
            case DsvAudioSDKConstants.DAB_SOURCE:
            case DsvAudioSDKConstants.LOCAL_RADIO_SOURCE:
                ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.CHANGE_COLLECT, ChangeReasonData.AIDL,
                        ModuleRadioTrigger.getInstance().mGetRadioStatusTool.getCurrentRadioMessage());
                break;
        }
    }
        ...
        switch (source) {
            case DsvAudioSDKConstants.FM_SOURCE:
                ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.OPEN_RADIO,
                        "mode".equals(reason) ? ChangeReasonData.MODE_CONTROL: ChangeReasonData.BOOT_RESUME, ModuleRadioTrigger.getInstance().mGetRadioStatusTool.getFMRadioMessage());
            ...
            case DsvAudioSDKConstants.AM_SOURCE:
                ...
                ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.OPEN_RADIO,
                        "mode".equals(reason) ? ChangeReasonData.MODE_CONTROL: ChangeReasonData.BOOT_RESUME, ModuleRadioTrigger.getInstance().mGetRadioStatusTool.getAMRadioMessage());
                if ("mode".equals(reason)){
            ...
            case DsvAudioSDKConstants.DAB_SOURCE:
                ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.OPEN_RADIO,
                        "mode".equals(reason) ? ChangeReasonData.MODE_CONTROL: ChangeReasonData.BOOT_RESUME, ModuleRadioTrigger.getInstance().mGetRadioStatusTool.getDABRadioMessage());
                if ("mode".equals(reason)){
    ...
    public byte[] getCurrentPic(String source){
        ...
        if (DsvAudioSDKConstants.DAB_SOURCE.equals(source)){
            DABMessage dabMessage = ModuleRadioTrigger.getInstance().mGetRadioStatusTool.getDABRadioMessage().getDabMessage();
            if (dabMessage != null){
                return dabMessage.getLogoDataList() == null ? new byte[0] : dabMessage.getLogoDataList();
            }
            return new byte[0];
        }
        }
