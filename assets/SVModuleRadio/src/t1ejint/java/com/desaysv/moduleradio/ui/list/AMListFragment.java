...
public class AMListFragment extends BaseFragment {
    ...
    public void initData() {
        playListAdapter.updateList(RadioList.getInstance().getAMEffectRadioMessageList());
    }
    ...
    public void updateList(){
        if (RadioList.getInstance().getAMEffectRadioMessageList().size() < 1){
    ...
    private static class MyHandler extends Handler {
        WeakReference<AMListFragment> weakReference;

        MyHandler(AMListFragment listFragment) {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            final AMListFragment listFragment = weakReference.get();
            // 只有界面显示了，才进行接收数据
