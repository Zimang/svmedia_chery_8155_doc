...
public class FMCollectListFragment extends BaseFragment {
    ...
    public void initData() {
        if (RadioList.getInstance().getFMCollectRadioMessageList().size() < 1){
            ...
            playListAdapter.updateList(RadioList.getInstance().getFMCollectRadioMessageList());
        }
    }
    ...
    private static class MyHandler extends Handler {
        WeakReference<FMCollectListFragment> weakReference;

        MyHandler(FMCollectListFragment listFragment) {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            final FMCollectListFragment listFragment = weakReference.get();
            // 只有界面显示了，才进行接收数据
