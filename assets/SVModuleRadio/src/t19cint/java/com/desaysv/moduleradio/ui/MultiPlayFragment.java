...
public class MultiPlayFragment extends BaseFragment implements View.OnClickListener, IDABOperationListener, CompoundButton.OnCheckedChangeListener {
    ...
    public int getLayoutResID() {
        return isRightRudder ?  R.layout.fragment_multiplay_layout_right : R.layout.fragment_multiplay_layout;
    }
    ...
    public void initView(View view) {
        ...
        rlDABPlayList.setItemAnimator(null);
        ...
        etRadioSc.setTextColor(getResources().getColor(R.color.radio_text_sub_color));
    ...
    public void initViewListener() {
        ...
        ivDABPause.setAccessibilityDelegate(playPauseAccessibilityDelegate);
        ivDABPause.setOnClickListener(this);
        ivDABNext.setOnClickListener(this);
        ivDABSearch.setOnClickListener(this);
        ivDABLike.setAccessibilityDelegate(likeAccessibilityDelegate);
    ...
    private final View.AccessibilityDelegate playPauseAccessibilityDelegate = new View.AccessibilityDelegate() {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfo info) {
            ...
            Log.d(TAG, "onInitializeAccessibilityNodeInfo ivDABPause");
            info.setChecked(!ivDABPause.isSelected());
        }
    };

    private final View.AccessibilityDelegate likeAccessibilityDelegate = new View.AccessibilityDelegate() {
        @Override
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfo info) {
            super.onInitializeAccessibilityNodeInfo(host, info);
            Log.d(TAG, "onInitializeAccessibilityNodeInfo ivDABLike");
            info.setChecked(ivDABLike.isSelected());
        }
    };
    ...
    public void onClick(View v) {
        ...
        if (R.id.ivDABPre == id){
            ...
            DABTrigger.getInstance().mRadioControl.processCommand(isRightRudder ? RadioAction.MULTI_SEEK_FORWARD : RadioAction.MULTI_SEEK_BACKWARD, ChangeReasonData.CLICK);
            ...
            //next
            updateCurrentScrollState();
            DABTrigger.getInstance().mRadioControl.processCommand(isRightRudder ? RadioAction.MULTI_SEEK_BACKWARD :  RadioAction.MULTI_SEEK_FORWARD,ChangeReasonData.CLICK);
     ...
     * @param radioMessage
     */
    private boolean toAmPlayPageIfNeed(RadioMessage radioMessage) {
        Log.d(TAG,"toAmPlayPageIfNeed: radioMessage = " + radioMessage);
        boolean toAmPlayPage = false;
        if (SPUtlis.getInstance().getIsShowCollectListMode()) {
            if (radioMessage.getRadioType() == RadioMessage.FM_AM_TYPE && radioMessage.getRadioBand() == RadioManager.BAND_AM) {
                Log.d(TAG,"toAmPlayPageIfNeed: openAMPlayPage");
                mHandler.sendEmptyMessageDelayed(RadioConstants.MSG_OPEN_AM_PLAY_PAGE, 150);
                toAmPlayPage = true;
            }
        }
        return toAmPlayPage;
...
//        updateDABSettings();
    ...
    private void updateCurrentRadio(){
        ...
        boolean toAmPlayPage = toAmPlayPageIfNeed(radioMessage);
        if (toAmPlayPage){
            return;
        }
        if (radioMessage.getRadioType() == RadioMessage.FM_AM_TYPE){
            tvMultiType.setText("FM");
            tvRadioPlayUnit.setVisibility(View.GONE);
            ...
            if (radioName != null && radioName.trim().length() > 1){
                ...
                tvRadioPlayUnit.setVisibility(View.VISIBLE);
            }
...
//                   }
            ...
            tvRadioText.setText(radioMessage.getDabMessage().getShortEnsembleLabel());
        }
    ...
    private boolean needScrollPosition(RadioMessage radioMessage){
        return currentScrollState == RecyclerView.SCROLL_STATE_IDLE || isNotSameRadio(radioMessage);
    }

    /**
     * 当前和preRadioMessage是否不是同一个
     * @param radioMessage
     * @return
     */
    private boolean isNotSameRadio(RadioMessage radioMessage) {
    ...
    public void updateRDSFLag(RDSFlagInfo info) {
        ...
        if (info != null && DABTrigger.getInstance().mRadioStatusTool.getRDSSettingsSwitchStatus() != null) {
            if (DABTrigger.getInstance().mRadioStatusTool.getRDSSettingsSwitchStatus() != null && DABTrigger.getInstance().mRadioStatusTool.getRDSSettingsSwitchStatus().getTa() == 1) {
    ...
    public void updateViewWithRDSSettingsChanged(RDSSettingsSwitch rdsSettingsSwitch) {
        ...
        if (rdsSettingsSwitch.getRds() == 1 && DABTrigger.getInstance().mRadioStatusTool.getCurrentRadioMessage().getRadioBand() == RadioManager.BAND_FM) {//RDS开关打开
            ...
            //RDS TF开关打开且当前电台有AF
            if (rdsSettingsSwitch.getAf() == 1 && DABTrigger.getInstance().mRadioStatusTool.getCurrentRDSFlagInfo() != null && DABTrigger.getInstance().mRadioStatusTool.getCurrentRDSFlagInfo().getAf() == 1) {
    ...
    private void updateEffectList(){
        Log.d(TAG,"updateEffectList: IsShowCollectListMode = " + SPUtlis.getInstance().getIsShowCollectListMode());
        ...
        if (SPUtlis.getInstance().getIsShowCollectListMode()) {
            ...
            //显示收藏列表
            currentList = RadioList.getInstance().getAllCollectRadioMessageList();
        } else {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what){
                ...
                case RadioConstants.MSG_UPDATE_RDS_SETTINGS:
                    ...
                    break;
                case RadioConstants.MSG_OPEN_AM_PLAY_PAGE:
                    multiPlayFragment.openAMPlayPage();
                    break;
                default:
                    break;
            }
