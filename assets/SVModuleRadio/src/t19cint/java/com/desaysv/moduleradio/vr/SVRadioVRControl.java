...
public class SVRadioVRControl {
    ...
    public void init(){
        ...
        isEUVersion = ProductUtils.isEUVersionByPartNum();
    }
                ...
                switch (event.getId()) {
                    ...
                    case VDEventVR.VR_MEDIA:
                        ...
                        if (RadioControlTool.getInstance().checkCurrentRadioSource()){
                            ...
                            Log.i(TAG,"on VR_MEDIA,key:"+key1);
                            // 内部Json语义表定义的Josn数据, 例: {"action":"OPEN","position":"F","type":"","value":""}
                            String data1 = param1.getValue();
                            Log.i(TAG,"on VR_MEDIA,data:"+data1);
    ...
    private static void handlePlayByBand(SVRadioVRBean svRadioVRBean){
        Log.i(TAG,"handlePlayByBand,svRadioVRBean:"+svRadioVRBean);
    ...
    private static void dealCloseRadio(){
        Log.i(TAG,"dealCloseRadio begin");
        boolean isRadioTop = isRadioTop();
        boolean isPlaying = ModuleRadioTrigger.getInstance().mGetRadioStatusTool.isPlaying();
        if (isRadioTop){
            Log.i(TAG,"dealCloseRadio isRadioTop");
            if (isPlaying){
                Log.i(TAG,"dealCloseRadio isRadioTop && isPlaying");
            ...
            if (isPlaying){
                Log.i(TAG,"dealCloseRadio isPlaying");
                ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.PAUSE, ChangeReasonData.VR_CONTROL);
                handleResponseTTS(R.string.radio_media_radio_opencollect_opened);
            } else {
                if (isEUVersion) {
                    handleResponseTTS(R.string.radio_media_radio_close_no_radio_playing);
                }
            }
        }
    }
    ...
    private static boolean isRadioTop() {
        ...
        Log.i(TAG, "getTopActivityPackageName: topPackageName = " + topPackageName);
    ...
    private static void dealPlay(String type,String value){
        Log.i(TAG,"dealPlay，type："+type+"value："+value);
        if (value != null && value.length() > 0){//确认是否带频点参数
            RadioMessage radioMessage = new RadioMessage(covertTypeToBand(type),covertValueToFreq(type, value), false, true);
            if (hasMulti){
                if (radioMessage.getRadioBand() == RadioManager.BAND_AM){
                    if (hasAM){
                        ...
                        //没有AM 播放AM电台
                        handleResponseTTS(R.string.radio_media_radio_bottom_line);
                        return;
                    }
                }
            ...
            Log.i(TAG,"dealPlay，targetState："+targetState);
    ...
    private static void handleScanBand(SVRadioVRBean svRadioVRBean){
        Log.i(TAG,"handleScanBand,svRadioVRBean:"+svRadioVRBean);
        if (svRadioVRBean.getSemantic() == null){
            return;
        }
        String action = svRadioVRBean.getSemantic().getAction();
    ...
    private static void dealScan(String type){
        Log.i(TAG,"dealScan，type："+type+"value");
    ...
    private static void handleControlCollect(SVRadioVRBean svRadioVRBean){
        Log.i(TAG,"handleControlCollect,svRadioVRBean:"+svRadioVRBean);
        if (svRadioVRBean.getSemantic() == null){
            return;
        }
        String action = svRadioVRBean.getSemantic().getAction();
    ...
    private static void dealPlayCollect(String type){
        ...
        if (RadioList.getInstance().getAllCollectRadioMessageList().size() > 0){
            ...
            //列表收藏为空，回复+随机播放一个电台  这里播放第一个有效电台
            dealPlayCollectWhenEmpty(type);
        }

    }

    /**
     * 列表收藏为空，回复+随机播放一个电台  这里播放第一个有效电台,没有有效电台则播放当前电台
     * @param type
     */
    private static void dealPlayCollectWhenEmpty(String type) {
        RadioMessage radioMessage = null;
        if (SVRadioVRConstant.Type.FM.equals(type)){
            if (!RadioList.getInstance().getFMEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getFMEffectRadioMessageList().get(0);
            }
        } else if (SVRadioVRConstant.Type.AM.equals(type)){
            if (!RadioList.getInstance().getAMEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getAMEffectRadioMessageList().get(0);
            }
        } else if (SVRadioVRConstant.Type.DAB.equals(type)){
            if (!RadioList.getInstance().getDABEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getDABEffectRadioMessageList().get(0);
            }
        }
        //没有匹配类型找到第一个有效电台
        if (radioMessage == null) {
            if (!RadioList.getInstance().getFMEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getFMEffectRadioMessageList().get(0);
            } else if (!RadioList.getInstance().getAMEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getAMEffectRadioMessageList().get(0);
            } else if (!RadioList.getInstance().getDABEffectRadioMessageList().isEmpty()){
                radioMessage = RadioList.getInstance().getDABEffectRadioMessageList().get(0);
            }
        }
        //没有有效电台则播放当前电台
        if (radioMessage == null) {
            radioMessage = ModuleRadioTrigger.getInstance().mGetControlTool.getStatusTool().getCurrentRadioMessage();
        }
        ModuleRadioTrigger.getInstance().mRadioControl.processCommand(RadioAction.OPEN_RADIO, ChangeReasonData.VR_CONTROL, radioMessage);
    ...
    private static void dealCollect(boolean collect){
        boolean isPlaying = ModuleRadioTrigger.getInstance().mGetRadioStatusTool.isPlaying();
        Log.i(TAG,"dealCollect,isPlaying:"+isPlaying);
        if (!isPlaying){//没有在播放
    ...
    private static void handleControlPlay(SVRadioVRBean svRadioVRBean){
        Log.i(TAG,"handleControlPlay,svRadioVRBean:"+svRadioVRBean);
        if (svRadioVRBean.getSemantic() == null){
            return;
        }
        String action = svRadioVRBean.getSemantic().getAction();
        ...
        Log.i(TAG,"handleControlPlay,currentRadio:"+currentRadio);
        switch (action){
            ...
            case SVRadioVRConstant.Action.PLAY:
                if (value != null && value.length() > 0){//按照协议，value不为空，说明需要播放第value个电台
                    ...
                    Log.i(TAG,"handleControlPlay,sourceName:"+sourceName+",position:"+position);
                    switch (sourceName){
                        case DsvAudioSDKConstants.DAB_SOURCE:
                            if (position >= RadioList.getInstance().getDABEffectRadioMessageList().size()){
                                Log.d(TAG,"position >= getDABEffectRadioMessageList().size.");
                                handleResponseTTS(R.string.radio_media_radio_amfreq_overrange);
                                return;
                            }
                        ...
                        case DsvAudioSDKConstants.AM_SOURCE:
                            ...
                            if (position >= RadioList.getInstance().getAMEffectRadioMessageList().size()){
                                Log.d(TAG,"position >= getAMEffectRadioMessageList().size.");
                                handleResponseTTS(R.string.radio_media_radio_amfreq_overrange);
                                return;
                            }
                        ...
                        case DsvAudioSDKConstants.FM_SOURCE:
                            if (position >= RadioList.getInstance().getFMEffectRadioMessageList().size()){
                                Log.d(TAG,"position >= getFMEffectRadioMessageList().size.");
                                handleResponseTTS(R.string.radio_media_radio_amfreq_overrange);
                                return;
                            }
    ...
    private static void handleControlPlayList(SVRadioVRBean svRadioVRBean){
        Log.i(TAG,"handleControlPlayList,svRadioVRBean:"+svRadioVRBean);
        if (svRadioVRBean.getSemantic() == null){
            return;
        }
        String action = svRadioVRBean.getSemantic().getAction();
        ...
        if (action.equals(SVRadioVRConstant.Action.OPEN)){
            ...
            if (CurrentShowFragmentUtil.isRadioPlayPageOnStart) {
                resId = R.string.radio_media_radio_opencollect_opened;
            } else {
     ...
     * @return
     */
    private static boolean isOnCollectListPage() {
        Log.d(TAG,"isOnCollectListPage: isRadioHomeListPageOnStart = " + CurrentShowFragmentUtil.isRadioHomeListPageOnStart);
        if (!CurrentShowFragmentUtil.isRadioHomeListPageOnStart) return false;
        Log.d(TAG,"isOnCollectListPage: getCurrentTab = " + SVRadioVRStateUtil.getInstance().getCurrentTab());
        boolean isOnCollectListPage = false;
        if (hasAM) {
            if (hasMulti) {
                if (SVRadioVRStateUtil.getInstance().getCurrentTab() == RadioConstants.TABWithMultiAM.POSITION_COLLECT) {
                    isOnCollectListPage = true;
                }
            } else {
                if (SVRadioVRStateUtil.getInstance().getCurrentTab() == RadioConstants.TABWithoutDAB.POSITION_COLLECT) {
                    isOnCollectListPage = true;
                }
            }
        } else {
            if (SVRadioVRStateUtil.getInstance().getCurrentTab() == RadioConstants.TABWithoutAM.POSITION_COLLECT) {
                isOnCollectListPage = true;
            }
        }
        return isOnCollectListPage;
    }
    ...
    private static void handleResponseTTS(int resId){
        ...
        Log.i(TAG,"handleResponseTTS:"+ ttsString);
        // TTS播报（set）
    ...
    private final IRadioStatusChange iRadioStatusChange = new IRadioStatusChange() {
        ...
        public void onCurrentRadioMessageChange() {
            Log.i(TAG,"onCurrentRadioMessageChange");
