...
public class RadioHomeFragment extends BaseFragment implements RadioBaseListFragment.IOnItemClickListener, DABFragment.IOnDABItemClickListener {
    ...
    private ImageView ivRadioListMask;
    ...
    public void initView(View view) {
        ...
        ivRadioListMask = view.findViewById(R.id.ivRadioListMask);
        if ("overlay2".equals(ProductUtils.getTheme(getContext()))){
            ivRadioListMask.setVisibility(View.VISIBLE);
        }else {
            ivRadioListMask.setVisibility(View.GONE);
        }
    ...
    public void setCurrentTab(int currentTab) {
        ...
        if (hasMulti){
            ...
            if (currentTab != RadioConstants.TABWithMultiAM.POSITION_MULTI){
                if (rlSearch != null && rlRadioMain != null) {
    ...
    public void initViewListener() {
        ...
        vpRadioMain.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                Log.d(TAG, "onPageSelected,position: " + position);
                SVRadioVRStateUtil.getInstance().setCurrentTab(position);
    ...
    private SearchView.OnQueryTextListener onQueryTextListener = new SearchView.OnQueryTextListener() {
        ...
        public boolean onQueryTextChange(String newText) {
            //输入法点击字母后，会先让用户预选，此时输入法会在输入框输入一个空格进行占位，
            //所以此处过滤输入的字母首尾空格
