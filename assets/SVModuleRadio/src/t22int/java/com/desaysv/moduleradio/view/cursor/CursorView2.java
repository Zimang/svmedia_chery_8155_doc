...
public abstract class CursorView2 extends View {
    ...
    public CursorView2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public CursorView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawCursorItem(Canvas canvas, int firstVisiblePosition, int positionOffset) {
        ...
        drawFrequencyLine(canvas, needDrawFrequency, positionOffset,position);
        // 是否需要在刻度线段下方绘制刻度值
     ...
     * @param positionOffset    当前要绘制的刻度与第一个可见刻度相差了几个刻度
     */
    protected abstract void drawFrequencyLine(Canvas canvas, boolean needDrawFrequency, int positionOffset, int position);
