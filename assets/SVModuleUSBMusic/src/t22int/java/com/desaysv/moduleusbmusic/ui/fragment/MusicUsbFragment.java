...
public class MusicUsbFragment extends MusicBaseFragment implements View.OnScrollChangeListener, ViewPager.OnPageChangeListener, View.OnClickListener {
    ...
    private void updateAdapterData() {
        if (musicListAdapter == null || musicFolderListAdapter == null) {
            initAdapter();
        } else {
            musicFolderListAdapter.setFolders(new HashMap<>(USBMusicDate.getInstance().getUSB1MusicAllMap()));
            musicListAdapter.setFileMessageList(new ArrayList<>(USBMusicDate.getInstance().getUSB1MusicAllList()));
        }
    }

    private void initAdapter() {
        Log.d(TAG, "initAdapter: create fragment");
        //初始化适配器
    ...
    public void downloadFiles(boolean showSelectView) {
        Log.d(TAG, "downloadFiles: isFolderType = " + isFolderType());
        if (isFolderType()) {
    ...
    public void switchToFile() {
        Log.d(TAG, "switchToFile: isFolderType = " + isFolderType);
        if (isFolderType) {
            if (rvMusicList == null || musicListAdapter == null) {
                Log.d(TAG, "switchToFile: rvMusicList = " + rvMusicList);
                return;
            }
            rvMusicList.setAdapter(musicListAdapter);
    ...
    public void switchToFolder() {
        Log.d(TAG, "switchToFolder: ");
        if (!isFolderType) {
            if (rvMusicList == null || musicFolderListAdapter == null) {
                Log.d(TAG, "switchToFolder: rvMusicList = " + rvMusicList);
                return;
            }
    ...
    private void updateUsbDeviceState() {
        Log.d(TAG, "updateUsbDeviceState: ");
    ...
    private final MusicFolderListAdapter.MusicListItemClickListener folderItemClickListener = new MusicFolderListAdapter.MusicListItemClickListener() {
        ...
        public void onFolderClick(FolderItem folderItem) {
            ...
            musicFolderListAdapter.notifyDataSetChanged();
            Log.d(TAG, "onFolderClick: " + folderItem.getNotePath());
            //如果是根目录则隐藏
        ...
        public void onSelectItemClick(int selectPosition, int selectSize, FolderItem folderItem) {
            Log.d(TAG, "onSelectItemClick: selectPosition = " + selectPosition + " selectSize = " + selectSize + " folderItem = " + folderItem);
    ...
    protected void dispatchMessage(Message msg, MusicBaseFragment fragment) {
        ...
        switch (msg.what) {
            case UPDATE_LIST:
                //查看当前是哪个列表状态
                if (usbFragment.musicListAdapter == null) {
                    usbFragment.initAdapter();
                }
            ...
            case UPDATE_FOLDER_LIST:
                //需要看当前是否为文件夹列表
                if (usbFragment.musicFolderListAdapter == null) {
                    usbFragment.initAdapter();
                }
            ...
            case UPDATE_SONG_INFO:
                updateDownloadState();
                if (usbFragment.musicListAdapter == null || usbFragment.musicFolderListAdapter == null) {
                    usbFragment.initAdapter();
                }
    ...
    private void showNoContentView() {
        boolean usb1Connect = DeviceStatusBean.getInstance().isUSB1Connect();
        Log.d(TAG, "showNoContentView: usb1Connect = " + usb1Connect);
        if (usb1Connect) {
    ...
    private void showSwitchFileModeAndDownloadSelect() {
        Log.d(TAG, "showSwitchFileModeAndDownloadSelect: ");
        //如果下载栏显示，则全部隐藏
    ...
    private void doDownloads() {
        ...
        for (Map.Entry<String, FileMessage> map : selectedMap.entrySet()) {
            if (map.getValue() != null) {
                ...
                //说明是文件夹里包含的文件
                List<FolderItem> folderItems = musicFolderListAdapter.getFolderMap().get(map.getKey());
                if (folderItems != null) {
        ...
        Log.d(TAG, "doDownloads: fileMessages = " + fileMessages);
    ...
    private void updateDownloadAndFileModeEnableStatus(boolean isEnable) {
        Log.d(TAG, "updateDownloadAndFileModeEnableStatus: isEnable = " + isEnable + " isCopying = " + CopyDeleteControl.getInstance().getCopyControl().isCopying());
