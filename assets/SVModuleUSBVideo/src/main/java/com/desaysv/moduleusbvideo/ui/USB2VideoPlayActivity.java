 ...
 * Comment USB2视频的播放界面
 */
public class USB2VideoPlayActivity extends BaseVideoPlayActivity {
    private static final String TAG = "USB2VideoPlayActivity";

    public static void startUSB2VideoPlayActivity(Context context, String path) {
        Log.d(TAG, "startUSB2VideoPlayActivity:");
        startUSB2VideoPlayActivity(context, true, path);
    }

    public static void startUSB2VideoPlayActivity(Context context, boolean isNeedToAutoPlay, String path) {
        Log.d(TAG, "startUSB2VideoPlayActivity: isNeedToAutoPlay = " + isNeedToAutoPlay + " path = " + path);
        Intent intent = new Intent(context, USB2VideoPlayActivity.class);
    ...
    protected void setPlayList() {
        int styleType = Objects.requireNonNull(VideoFileListTypeTool.getInstance(DeviceConstants.DevicePath.USB1_PATH)).getStyleType();
        Log.d(TAG, "setPlayList: autoPlay: styleType = " + styleType);
        if (VideoFileListTypeTool.STYLE_TYPE_ALL == styleType) {
            getVideoControl().setPlayList(USBVideoDate.getInstance().getUSB2VideoAllList(), CurrentPlayListType.ALL);
        } else {
            List<FileMessage> usb2VideoFolderPlayList = USBVideoFolderListData.getInstance().getUSB2VideoFolderPlayList(currentPath);
            getVideoControl().setPlayList(usb2VideoFolderPlayList, CurrentPlayListType.FLODER);
        }
    }

    @Override
    protected void initVideoControl() {
        Log.i(TAG, "initVideoControl: USB2");
        ModuleUSBVideoTrigger.getInstance().initUSB2VideoControl(requestVideoPlayer);
    }
    ...
    protected IStatusTool getVideoStatusTool() {
        return ModuleUSBVideoTrigger.getInstance().USB2VideoStatusTool;
    }

    @Override
    protected void registerVideoStatusChangeListener(IMediaStatusChange iMediaStatusChange) {
        Objects.requireNonNull(ModuleUSBVideoTrigger.getInstance().getUSB2VideoControlTool)
    ...
    private final DeviceListener deviceListener = new DeviceListener() {
        ...
        public void onDeviceStatusChange(String path, boolean status) {
            ...
            if (DeviceConstants.DevicePath.USB0_PATH.equals(path) && !status) {
                // U盘拔出，需要先释放播放器
                getVideoControl().processCommand(MediaAction.STOP, ChangeReasonData.USB2_EJECT);
                Log.e(TAG, "onDeviceStatusChange: FINISH 2");
                finish();
            }
        }
    };
